...
#define FIREBASE_FIRMWAREVERSION    "v00.00.01"
#define FIREBASE_APPNAME    "sampleAppName"
...
//#define FIREBASE_READKEY    "lastFirmwareVersion"
// #define FIREBASE_READKEY2   "firmwareURL"
#define FIREBASE_URL_WRITE  "https://smartsocket-97183-default-rtdb.firebaseio.com/"FIREBASE_APPNAME"/"FIREBASE_WRITEKEY".json?auth="FIREBASE_AUTH
#define FIREBASE_URL_READ   "https://smartsocket-97183-default-rtdb.firebaseio.com/"FIREBASE_APPNAME"/"FIREBASE_READKEY".json?auth="FIREBASE_AUTH
...
static EventGroupHandle_t wifi_event_group;
 ...
 int state_value = 0; // Okunan state değerini saklayacak değişken
 int adc_value = 0;
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA:
            if (!esp_http_client_is_chunked_response(evt->client)) {
                ...
                // Okunan veriyi buffer'a kaydediyoruz
                // if (evt->data_len < sizeof(firebase_read_data)) {
                //     strncpy(firebase_read_data, (char*)evt->data, evt->data_len);
                //     firebase_read_data[evt->data_len] = '\0'; // String sonlandırıcısını ekleyin
                // } else {
                //     ESP_LOGW(TAG, "Received data is too large, truncating");
                //     strncpy(firebase_read_data, (char*)evt->data, sizeof(firebase_read_data) - 1);
                //     firebase_read_data[sizeof(firebase_read_data) - 1] = '\0';
                // }

                // Okunan veriyi integer değere çevir
                state_value = atoi(evt->data); // `state` değerini kaydet
...
static void write_owndata_to_firebase(void *pvParameter)
{
    ...
    char mac_str[18]; // MAC adresi için yeterli uzunlukta string
    snprintf(mac_str, sizeof(mac_str), "%02X:%02X:%02X:%02X:%02X:%02X",
             mac_addr[0], mac_addr[1], mac_addr[2],
             mac_addr[3], mac_addr[4], mac_addr[5]);
    // current++;
    char put_data[200]; // Yeterli uzunlukta bir buffer
    snprintf(put_data, sizeof(put_data), "{\"currentValue\": \"%d\", \"macAddr\": \"%s\"}",
             adc_value, mac_str);
...
static void read_data_from_firebase(void *pvParameter)
{
    ...
    if (err == ESP_OK) {
        ...
        // Okunan state değerine göre LED'i kontrol et
            control_led(state_value);
    } else {
